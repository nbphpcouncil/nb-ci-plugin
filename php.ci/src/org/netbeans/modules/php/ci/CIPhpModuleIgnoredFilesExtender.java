 ...
 */
public class CIPhpModuleIgnoredFilesExtender extends PhpModuleIgnoredFilesExtender {
    private final PhpModule phpModule;
    // currently, only "cache" directory can be hidden
    private final File cache;    
    
    public CIPhpModuleIgnoredFilesExtender(PhpModule phpModule) {
        assert phpModule != null;

        this.phpModule = phpModule;

        FileObject cacheFO = CIPhpFrameworkProvider.locate(phpModule, CIPhpFramework.DIRECTORY_CACHE, true);
        
        if (cacheFO != null && cacheFO.isFolder()) {
            cache = FileUtil.toFile(cacheFO);
        } else {
            cache = new File(FileUtil.toFile(phpModule.getSourceDirectory()), CIPhpFramework.getApplicationCacheDirectoryPath());
        }
    }
